...
class Week implements Comparable<Week> {
  const Week(this.weekBasedYear, this.weekOfYear) : assert(weekOfYear >= 1 && weekOfYear <= 53);
  ...
  factory Week.forDate(DateTime date) {
    ...
    if (weekOfYear == 0) {
      ...
      final weekOfYear = DateTimeTimetable.date(year - 1, 12, 31).week.weekOfYear;
      return Week(year - 1, weekOfYear);
    }

    if (weekOfYear == 53 && DateTime(year, 12, 31).weekday < DateTime.thursday) {
  ...
  DateTime getDayOfWeek(int dayOfWeek) {
    ...
    final yearCorrection = DateTimeTimetable.date(weekBasedYear, 1, 4).weekday + 3;
    return DateTimeTimetable.date(weekBasedYear, 1, 1) + (base - yearCorrection - 1).days;
  }
  ...
  bool operator ==(Object other) {
    return other is Week && other.weekBasedYear == weekBasedYear && other.weekOfYear == weekOfYear;
  }
