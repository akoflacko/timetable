...
class TimeController extends ValueNotifier<TimeRange> {
  TimeController({
    ...
    this.minDayHeight,
        ...
        assert(
          maxDuration == null || initialRange == null || initialRange.duration <= maxDuration,
        ),
        assert(maxRange == null || minDuration <= maxRange.duration),
        assert(
          maxDuration == null || maxRange == null || maxDuration <= maxRange.duration,
        ),
  ...
  static TimeRange _getInitialRange(
    ...
    if (maxDuration != null && maxRange != null && maxDuration <= maxRange.duration) {
  ...
  bool _isValidRange(TimeRange range) {
    ...
    return actualMinDuration <= range.duration && range.duration <= actualMaxDuration;
  }
  ...
  Duration get actualMinDuration => minDuration.coerceAtMost(maxDurationFromMinDayHeightOrDefault);

  /// The maximum visible duration when zooming out.
  final Duration maxDuration;

  /// The maximum visible duration, honoring [maxDuration] and [minDayHeight].
  Duration get actualMaxDuration => maxDuration.coerceAtMost(maxDurationFromMinDayHeightOrDefault);
  ...
  double? get minClientHeight => _clients.map((it) => it.height).minOrNull?.toDouble();
  ...
  void _updateMaxDurationFromMinDayHeight() {
    ...
    final minRangeHeight = minDayHeight! * (maxRange.duration / maxPossibleDuration);
    _maxDurationFromMinDayHeight = maxRange.duration * (minClientHeight / minRangeHeight);
  }

  Duration get maxDurationFromMinDayHeightOrDefault => maxDurationFromMinDayHeight ?? maxDuration;
  ...
  Future<void> animateTo(
    ...
    _animationController = AnimationController(debugLabel: 'TimeController', vsync: vsync)
      ..addListener(() {
