...
class _TimeZoomState extends State<TimeZoom> with SingleTickerProviderStateMixin {
  ...
  void _onScaleUpdate(ScaleUpdateDetails details) {
    ...
    if (rawScale <= precisionErrorTolerance) {
      ...
      newDuration = (_initialRange!.duration * (1 / rawScale)).coerceIn(controller.actualMinDuration, controller.actualMaxDuration);
    }
  ...
  void _onScaleEnd(ScaleEndDetails details) {
    ...
    _animation = Tween(begin: _outerOffset, end: frictionSimulation.finalX).animate(
...
class _ScaleGestureRecognizer extends OneSequenceGestureRecognizer {
  ...
  void _updateLines() {
    ...
    } else if (_initialLine != null && _initialLine!.pointerStartId == _pointerQueue.first && _initialLine!.pointerEndId == _pointerQueue.second) {
  ...
  void _advanceStateMachine(
    ...
    if (_state == _ScaleState.possible) {
      ...
      if (spanDelta > computeScaleSlop(pointerDeviceKind) || focalPointDelta > computeHitSlop(pointerDeviceKind, gestureSettings)) {
