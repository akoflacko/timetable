...
class WeekIndicator extends StatelessWidget {
  ...
  TextStyle _getEffectiveTextStyle(BuildContext context, TextStyle textStyle) {
    ...
    if (effectiveTextStyle.inherit) {
      effectiveTextStyle = context.defaultTextStyle.style.merge(effectiveTextStyle);
    }
...
class _RenderWeekIndicatorText extends RenderBox {
  ...
  @override
  double computeMinIntrinsicWidth(double height) => _labelPainters.map((it) => it.width).min.toDouble();
  @override
  ...
  @override
  double computeMinIntrinsicHeight(double width) => _labelPainters.map((it) => it.height).min.toDouble();
  @override
  ...
  void performLayout() {
    ...
    if (alwaysUseNarrowestVariant) {
      ...
      _labelPainter = _labelPainters
              .where(
                (it) => constraints.minWidth <= it.size.width && it.size.width <= constraints.maxWidth,
              )
              .firstOrNull ??
          narrowestPainter();
    }
  ...
  @override
  void paint(PaintingContext context, Offset offset) => _labelPainter.paint(context.canvas, offset);
}
...
class WeekIndicatorStyle {
  factory WeekIndicatorStyle(
    ...
    return WeekIndicatorStyle.raw(
      ...
      textStyle: textStyle ?? context.textTheme.bodyMedium!.copyWith(color: colorScheme.surface.mediumEmphasisOnColor),
