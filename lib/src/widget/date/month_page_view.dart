...
class _MonthPageViewState extends State<MonthPageView> {
  ...
  void initState() {
    super.initState();
    _controller = widget.monthPageController ?? MonthPageController(initialMonth: DateTimeTimetable.currentMonth());
...
class MonthPageController extends ChangeNotifier implements ValueListenable<DateTime> {
  MonthPageController({required DateTime initialMonth})
      : assert(initialMonth.debugCheckIsValidTimetableMonth()),
        _pageController = PageController(initialPage: _pageFromMonth(initialMonth)) {
  ...
  static final _minPage = (_minMonth.year * DateTime.monthsPerYear) + (_minMonth.month - 1);
  static DateTime _monthFromPage(int page) {
    page = _minPage + page;
    final year = (page < 0 ? page - DateTime.monthsPerYear + 1 : page) ~/ DateTime.monthsPerYear;
  ...
  void jumpTo(DateTime month) => _pageController.jumpToPage(_pageFromMonth(month));
}
