...
class MultiDateEventHeaderStyle {
  ...
  bool operator ==(Object other) {
    return other is MultiDateEventHeaderStyle && eventHeight == other.eventHeight && maxEventRows == other.maxEventRows && padding == other.padding;
  }
}
...
class _MultiDateEventHeaderEventsState<E extends Event> extends State<_MultiDateEventHeaderEvents<E>> {
  ...
  void _updateEventPositions({required int? oldMaxEventRows}) {
    ...
    _yPositions.removeWhere((event, yPosition) {
      return event.start.page.floor() > widget.pageValue.lastVisiblePage || event.end.page.ceil() <= widget.pageValue.firstVisibleDate.page;
    });
    ...
    for (final date in widget.pageValue.visibleDatesIterable) {
      ...
      final maxEventPosition =
          _yPositions.entries.where((it) => it.key.interval.intersects(dayInterval)).map((it) => it.value ?? widget.maxEventRows).maxOrNull;
  ...
  Widget _buildEvent(AllDayEventBuilder<E> allDayBuilder, E event) {
    return allDayBuilder(
      ...
      AllDayEventLayoutInfo(
        ...
        hiddenEndDays: (event.end.page.ceil() - widget.pageValue.page - widget.pageValue.visibleDayCount).coerceAtLeast(0),
      ),
    );
  }
}
